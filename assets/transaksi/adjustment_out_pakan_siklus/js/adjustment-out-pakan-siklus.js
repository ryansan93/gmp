var aops = {
	...
	setSelect2Plasma: function(elm, all = null) {
        $(elm).select2({
            ajax: {
                url: 'transaksi/AdjustmentOutPakanSiklus/getPlasma',
                dataType: 'json',
                type: 'GET',
	...
	setSelect2Noreg: function(elm, all = null) {
        $(elm).select2({
            ajax: {
                url: 'transaksi/AdjustmentOutPakanSiklus/getNoreg',
                dataType: 'json',
                type: 'GET',
                data: function (params, jenis) {
                    var query = {
    ...
    settingUp: function() {
		...
		$(document).ready(function () {
            aops.setSelect2Plasma( $('#riwayat').find('.mitra'), 1 );
            aops.setSelect2Plasma( $('#action').find('.mitra') );
            aops.setSelect2Noreg( $('#action').find('.noreg') );
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url: 'transaksi/AdjustmentOutPakanSiklus/getLists',
                data: { 'params': params },
                type: 'GET',
                dataType: 'HTML',
	...
	loadForm: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/AdjustmentOutPakanSiklus/loadForm',
            data: { 'params': params },
            type: 'GET',
            dataType: 'HTML',
            beforeSend: function(){ showLoading() },
	...
	getStokSiklus: function() {
		...
		if ( err > 0 ) {
			...
			$.ajax({
	            url: 'transaksi/AdjustmentOutPakanSiklus/getStokSiklus',
	            data: { 'params': params },
	            type: 'POST',
	            dataType: 'JSON',
	            beforeSend: function(){ showLoading() },
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function (result) {
				if ( result ) {
					...
					$.ajax({
			            url: 'transaksi/AdjustmentOutPakanSiklus/save',
			            data: { 'params': params },
			            type: 'POST',
			            dataType: 'JSON',
			            beforeSend: function(){ showLoading() },
			            success: function(data){
			            	...
			            	if ( data.status == 1 ) {
			            		aops.execHitStokSiklus(data.content);
			            	} else{
			            		hideLoading();
			            		bootbox.alert(data.message);
			            	}
	...
	delete: function(elm) {
		...
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function (result) {
			if ( result ) {
				...
				$.ajax({
					url: 'transaksi/AdjustmentOutPakanSiklus/delete',
					data: { 'params': params },
					type: 'POST',
					dataType: 'JSON',
					beforeSend: function(){ showLoading() },
	...
	execHitStokSiklus: function(content) {
		...
		$.ajax({
			url: 'transaksi/AdjustmentOutPakanSiklus/execHitStokSiklus',
			data: {
				'params': params
			},
			type: 'POST',
			...
			success: function(data) {
				hideLoading();
				if ( data.status == 1 ) {
					aops.execInsertJurnal(data.content);
				} else {
					bootbox.alert(data.message);
				};
			},
    ...
    execInsertJurnal: function(content) {
		...
		$.ajax({
			url: 'transaksi/AdjustmentOutPakanSiklus/execInsertJurnal',
			data: {
				'params': params
			},
			type: 'POST',
				...
				if ( data.status == 1 ) {
					bootbox.alert(data.content.message, function() {
                        if ( data.content.status == 3 ) {
                            aops.loadForm(null, null, 'action');
                        } else {
                            aops.loadForm(data.content.id, null, 'action');
                        }
                    });
				} else {
					bootbox.alert(data.message);
