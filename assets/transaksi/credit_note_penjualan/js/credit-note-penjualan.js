var cn = {
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url: 'transaksi/CreditNotePenjualan/getLists',
                data: { 'params': params },
                type: 'GET',
                dataType: 'HTML',
	...
	loadForm: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/CreditNotePenjualan/loadForm',
            data: { 'params': params },
            type: 'GET',
            dataType: 'HTML',
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function (result) {
				if ( result ) {
					var params = {
						...
						'nilai_cn': numeral.unformat($(div).find('.nilai_cn').val()),
						'supplier': $(div).find('.supplier').select2('val'),
                    ...
                    $.ajax({
                        url : 'transaksi/CreditNotePenjualan/save',
                        type : 'post',
                        data : formData,
                        beforeSend : function(){ showLoading() },
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin meng-edit data ?', function (result) {
				if ( result ) {
                    ...
                    $.ajax({
                        url : 'transaksi/CreditNotePenjualan/edit',
                        type : 'post',
                        data : formData,
                        beforeSend : function(){ showLoading() },
                        success : function(data){
    ...
    delete: function(elm) {
        ...
        bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function (result) {
            if ( result ) {
                ...
                $.ajax({
                    url : 'transaksi/CreditNotePenjualan/delete',
                    data: { 'params': params },
					type: 'POST',
					dataType: 'JSON',
                    beforeSend : function(){ showLoading() },
                    success : function(data){
                        ...
                        if ( data.status == 1 ) {
                            bootbox.alert(data.message, function() {
                                ...
                                $('a[data-tab=riwayat]').addClass('active');
                                $('div#riwayat').addClass('show active');
                                cn.getLists();
                            });
                        } else {
                            bootbox.alert(data.message);
