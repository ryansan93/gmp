var aovs = {
	...
	setSelect2Plasma: function(elm, all = null) {
        $(elm).select2({
            ajax: {
                url: 'transaksi/AdjustmentOutVoadipSiklus/getPlasma',
                dataType: 'json',
                type: 'GET',
	...
	setSelect2Noreg: function(elm, all = null) {
        $(elm).select2({
            ajax: {
                url: 'transaksi/AdjustmentOutVoadipSiklus/getNoreg',
                dataType: 'json',
                type: 'GET',
                data: function (params, jenis) {
                    var query = {
    ...
    settingUp: function() {
		...
		$(document).ready(function () {
            aovs.setSelect2Plasma( $('#riwayat').find('.mitra'), 1 );
            aovs.setSelect2Plasma( $('#action').find('.mitra') );
            aovs.setSelect2Noreg( $('#action').find('.noreg') );
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url: 'transaksi/AdjustmentOutVoadipSiklus/getLists',
                data: { 'params': params },
                type: 'GET',
                dataType: 'HTML',
	...
	loadForm: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/AdjustmentOutVoadipSiklus/loadForm',
            data: { 'params': params },
            type: 'GET',
            dataType: 'HTML',
            beforeSend: function(){ showLoading() },
	...
	getStokSiklus: function() {
		...
		if ( err > 0 ) {
			...
			$.ajax({
	            url: 'transaksi/AdjustmentOutVoadipSiklus/getStokSiklus',
	            data: { 'params': params },
	            type: 'POST',
	            dataType: 'JSON',
	            beforeSend: function(){ showLoading() },
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function (result) {
				if ( result ) {
					...
					$.ajax({
			            url: 'transaksi/AdjustmentOutVoadipSiklus/save',
			            data: { 'params': params },
			            type: 'POST',
			            dataType: 'JSON',
			            beforeSend: function(){ showLoading() },
			            success: function(data){
			            	...
			            	if ( data.status == 1 ) {
			            		aovs.execHitStokSiklus(data.content);
			            	} else{
			            		hideLoading();
			            		bootbox.alert(data.message);
			            	}
	...
	delete: function(elm) {
		...
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function (result) {
			if ( result ) {
				...
				$.ajax({
					url: 'transaksi/AdjustmentOutVoadipSiklus/delete',
					data: { 'params': params },
					type: 'POST',
					dataType: 'JSON',
					beforeSend: function(){ showLoading() },
	...
	execHitStokSiklus: function(content) {
		...
		$.ajax({
			url: 'transaksi/AdjustmentOutVoadipSiklus/execHitStokSiklus',
			data: {
				'params': params
			},
			type: 'POST',
			...
			success: function(data) {
				hideLoading();
				if ( data.status == 1 ) {
					aovs.execInsertJurnal(data.content);
				} else {
					bootbox.alert(data.message);
				};
			},
    ...
    execInsertJurnal: function(content) {
		...
		$.ajax({
			url: 'transaksi/AdjustmentOutVoadipSiklus/execInsertJurnal',
			data: {
				'params': params
			},
			type: 'POST',
				...
				if ( data.status == 1 ) {
					bootbox.alert(data.content.message, function() {
                        if ( data.content.status == 3 ) {
                            aovs.loadForm(null, null, 'action');
                        } else {
                            aovs.loadForm(data.content.id, null, 'action');
                        }
                    });
				} else {
					bootbox.alert(data.message);
