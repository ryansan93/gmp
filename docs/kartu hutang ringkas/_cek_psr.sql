...
-- GL NAIK per invoice: BBM (via no_order, invoice NULL) + SEMUA coa_asal=21180.200 ber-invoice (reklas BYR, memo MM, dll)
gln AS (
...
-- GL TURUN per invoice: SEMUA coa_tujuan=21180.200 ber-invoice
glt AS (
...
FROM fin
WHERE unit='PSR' AND ABS(selisih) > 0.001 AND ABS(selisih) < 100000
