...
rkredit AS (
  SELECT SUBSTRING(REPLACE(REPLACE(rpd.no_bayar,'INV/RHPP/G/',''),'INV/RHPP/',''),1,3) unit, SUM(rpd.transfer) kredit
  FROM realisasi_pembayaran_det rpd JOIN realisasi_pembayaran rp ON rpd.id_header=rp.id
  WHERE rpd.transaksi='PLASMA' AND rp.tgl_realisasi<@cut GROUP BY SUBSTRING(REPLACE(REPLACE(rpd.no_bayar,'INV/RHPP/G/',''),'INV/RHPP/',''),1,3)
),
