...
SELECT inv, unit, konfir, bayar_transfer, bayar_pph, bayar_cn, memo_settlement,
       gl_naik, gl_turun, CAST(op_saldo AS decimal(18,2)) op_saldo, CAST(gl_saldo AS decimal(18,2)) gl_saldo,
       CAST(selisih AS decimal(18,2)) selisih
FROM fin
WHERE unit='MLG' AND ABS(selisih) > 0.001 AND ABS(selisih) < 100
