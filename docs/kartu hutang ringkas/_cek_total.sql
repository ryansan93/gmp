...
glt AS (SELECT dj.invoice inv, SUM(dj.nominal) gl_turun FROM det_jurnal dj WHERE dj.coa_tujuan='21180.200' AND dj.invoice LIKE 'BYD/%' AND dj.tanggal<=@end GROUP BY dj.invoice),
calc AS (
  ...
  FROM konf k LEFT JOIN trf t ON t.inv=k.inv LEFT JOIN cn c ON c.inv=k.inv LEFT JOIN memo m ON m.inv=k.inv LEFT JOIN gln g ON g.inv=k.inv LEFT JOIN glt gt ON gt.inv=k.inv
),
fin AS (
  SELECT *, (konfir-(bayar_transfer+bayar_cn+bayar_pph+memo_settlement))-(gl_naik-gl_turun) AS selisih FROM calc
)
SELECT
  CASE
    WHEN inv LIKE 'BYD/05/26/%' AND ABS(selisih) < 500000 THEN '1. PPh-only invoice Mei (belum bayar)'
    WHEN gl_naik > konfir*1.5 THEN '2. Split-konfir / BBM dobel (gl_naik dobel)'
    WHEN memo_settlement > 0 THEN '3. Pasangan memo-settlement'
    ELSE '4. PPh / selisih kecil lainnya'
  END kategori,
  COUNT(*) n_inv, CAST(SUM(selisih) AS decimal(18,2)) total_selisih
FROM fin WHERE ABS(selisih) > 0.5
GROUP BY CASE
    WHEN inv LIKE 'BYD/05/26/%' AND ABS(selisih) < 500000 THEN '1. PPh-only invoice Mei (belum bayar)'
    WHEN gl_naik > konfir*1.5 THEN '2. Split-konfir / BBM dobel (gl_naik dobel)'
    WHEN memo_settlement > 0 THEN '3. Pasangan memo-settlement'
  ...
  END
ORDER BY kategori;
SELECT CAST(SUM(selisih) AS decimal(18,2)) GRAND_TOTAL_SELISIH FROM fin WHERE ABS(selisih)>0.5;
