...
memo AS (SELECT mi.no_invoice inv, SUM(mi.nilai) m FROM mmitem mi WHERE mi.coa_tujuan='21174.000' GROUP BY mi.no_invoice)
SELECT k.inv, k.unit, CAST(k.konfir AS decimal(18,2)) konfir,
   ...
   CAST(k.konfir - isnull(b.transfer,0) - isnull(b.cn,0) - isnull(mo.m,0) AS decimal(18,4)) sisa
FROM konf k LEFT JOIN byr b ON b.inv=k.inv LEFT JOIN memo mo ON mo.inv=k.inv
WHERE k.unit IN ('MJK','JBR') AND ABS(k.konfir - isnull(b.transfer,0) - isnull(b.cn,0) - isnull(mo.m,0)) BETWEEN 0.001 AND 0.999
