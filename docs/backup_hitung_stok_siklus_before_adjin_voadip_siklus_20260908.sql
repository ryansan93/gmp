/* ============================================================================
   BACKUP -- definisi `hitung_stok_siklus` di gmp_erp_live SEBELUM patch
   `docs/alter_hitung_stok_siklus_adjin_voadip_siklus.sql` diterapkan
   (2026-09-08). Diambil via `sys.sql_modules.definition` langsung dari
   server live (koneksi sqlsrv PHP, sama seperti proses pengambilan body
   untuk patch adjout_voadip_siklus sebelumnya -- OBJECT_DEFINITION() lewat
   tool grid text bisa kepotong utk objek sebesar ini, sys.sql_modules aman).

   Body ini SUDAH termasuk patch adjout_voadip_siklus (2026-09-04) --
   backup pra-patch UNTUK PATCH ITU ada terpisah di
   `docs/backup_hitung_stok_siklus_before_adjout_voadip_siklus_20260904.sql`.

   Patch adjin_voadip_siklus (2026-09-08) divalidasi sebelum diterapkan:
     1. SET PARSEONLY ON atas teks ALTER PROCEDURE penuh -- lolos, tidak ada
        syntax error.
     2. Smoke-test fungsional dalam BEGIN TRAN ... ROLLBACK (1 batch, ALTER
        via EXEC(@sql) dynamic SQL supaya bisa digabung sebatch dengan
        BEGIN TRAN/ROLLBACK -- literal ALTER PROCEDURE harus jadi satu-
        satunya statement dalam batch-nya kalau dieksekusi langsung):
          - ALTER PROCEDURE dengan body ter-patch berhasil (definition
            length berubah dari 259438 byte -> 260644 byte, sesuai insersi
            2 blok baru).
          - EXEC hitung_stok_siklus 'voadip', 'terima_voadip', '17711',
            '2026-09-08', 2, '26080200101', '' (cabang existing yg TIDAK
            disentuh patch ini) berhasil tanpa error -- regresi aman.
          - ROLLBACK dikonfirmasi: definition length kembali ke 259438 byte
            (persis body pra-patch di file ini).
     Setelah smoke-test lolos, ALTER PROCEDURE yang SAMA PERSIS di-apply lagi
     di luar transaksi (permanen) via `docs/alter_hitung_stok_siklus_adjin_voadip_siklus.sql`.

   Scope patch SEMPIT: 1 blok registrasi #lnoreg (`adjin_voadip_siklus`,
   disisipkan tepat setelah blok `adjout_voadip_siklus` yg sudah ada, baris
   ~2278-2284 versi ini) + 1 `union all` baru di blok DATA MASUK section
   voadip (disisipkan tepat sebelum `) dm`, baris ~2464-2467 versi ini),
   pola diadaptasi dari `adjin_doc` (straight insert per baris, TANPA
   FIFO/konsumsi lot, karena ini arah MASUK). Cabang lain (`terima_voadip`,
   `retur_voadip`, `adjout_voadip_siklus`) dan section `doc`/`pakan` TIDAK
   disentuh sama sekali.

   ROLLBACK (kalau patch adjin_voadip_siklus perlu dibatalkan): jalankan isi
   file ini apa adanya (ganti CREATE jadi ALTER PROCEDURE, karena objek pasti
   masih ada) ke gmp_erp_live.
   ============================================================================ */
					...
					BEGIN						
						...
						BEGIN
							...
							BEGIN
								...
								/*
								 * NOTE (fix 2026-08-11): kalau ada mutasi/pindah pakan (det_kirim_pakan.no_sj_asal)
								 * yang MEREFERENSIKAN batch ini dan BELUM tercatat di det_stok_trans_siklus, jatah itu
								 * "dipesan" duluan dan tidak boleh dimakan LHK -- supaya pas mutasinya benar2 dieksekusi
								 * (exact-match ke no_sj_asal di cabang non-LHK) stoknya masih ada. Adaptasi dari mekanisme
								 * #pp_pakan yang sudah ada di cabang lama (tgl_docin < 2026-05-01), diterapkan ke cabang baru.
								 */
								IF (
									EXISTS (
										select * 
										...
										left join
											...
											mutasi
											on
												REPLACE(mutasi.no_sj_asal, 'SJ', 'OP') = dss.kode_trans and
												mutasi.item = dss.kode_barang
										where 
											dss.noreg = @noreg and 
								...
								BEGIN
									select top 1
										...
										@ds_reserved = cast(isnull(mutasi.jumlah, 0) as decimal(13, 2))
									from det_stok_siklus dss
									left join
										(
											select 
									...
									IF ( @_dk_jumlah <= @ds_avail )
									 ...
									 BEGIN							 
										 insert into det_stok_trans_siklus (id_header, tgl_trans, kode_trans, jumlah, kode_barang, tbl_name)
										 values
										 (@ds_id, @dk_tgl_trans, @dk_kode_trans, @ds_avail, @dk_kode_barang, @dk_tbl_name)
										 
										 SET @_dk_jumlah = @_dk_jumlah - @ds_avail
										 SET @ds_jml_stok = @ds_jml_stok - @ds_avail
													...
													left join
														(
															...
															where
																...
																dsts.kode_barang = @ds_kode_brg and
																dsts.tbl_name = 'terima_pakan'
															group by
																dss.kode_trans, dsts.kode_barang
														) dsts
														on
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						ELSE
						BEGIN
							-- semua lot sudah 0 (atau minus) -- tetap catat kekurangannya di
							-- lot PALING BARU biar keliatan minus di laporan (kartu stok siklus
							-- dll), bukan didiamkan/dibuang. Ini permintaan eksplisit: adjustment
							-- boleh dorong stok siklus sampai negatif, tidak dibatasi safety-cap.
							IF ( EXISTS (
								select * from det_stok_siklus dss
								where
									dss.noreg = @noreg and
