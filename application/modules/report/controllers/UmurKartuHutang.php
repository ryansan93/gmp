...
class UmurKartuHutang extends Public_Controller {
    ...
    public function getData() {
        ...
        $where_jenis_hutang = null;
        if ( !empty($jenis_hutang) ) {
            $where_jenis_hutang = "where jenis_hutang in ('".implode("', '", $jenis_hutang)."')";
        }
        ...
        $sql = "
            ...
            /* NOCOUNT wajib di batch multi-statement begini -- tanpa ini, PDO driver sqlsrv (dipakai
               ...
               resultset SELECT terakhir yg sungguhan. */

            /* ============================================================================
               V2-PORTED: DOC + PAKAN + OVK debt/payment source logic, copied verbatim
               (renamed with v2_ prefix) from KartuHutangPerInvoiceV2.php getData(), same
               pattern already applied to KartuHutangPerInvoice.php. See that file's header
               comment for full rationale of every branch below.
               ============================================================================ */
            ...
            /* Pre-agregasi #v2_sumber_hutang per nomor invoice utk SALDO AWAL -- WAJIB, krn 1
               ...
               konsisten dgn seluruh cabang LAINNYA di inv_raw (semuanya sudah natural 1 baris/nomor). */
            ...
            from (
                ...
                (
                    ...
                    from (
                        select * from (
                        /* V2-PORTED: DOC + PAKAN + OVK saldo awal debt, pre-agregasi per nomor -- lihat #v2_saldo_inv di preamble */
                        select tanggal, nomor, supplier, total, jenis_hutang
                        from #v2_saldo_inv
                        /* END - V2-PORTED DOC/PAKAN/OVK */

                        union all

                        select kpop.tgl_bayar as tanggal, kpop.nomor, kpop.ekspedisi_id as supplier, kpop.total, 'LAINNYA' as jenis_hutang from konfirmasi_pembayaran_oa_pakan kpop
                        where
                            kpop.tgl_bayar < '".$start_date."'

                        union all

                        select kpp.tgl_bayar as tanggal, kpp.nomor, kpp.mitra as supplier, kpp.total, 'LAINNYA' as jenis_hutang from konfirmasi_pembayaran_peternak kpp
                        where
                        ...
                        /* INVOICE LEWAT MEMO (OVK) DIHAPUS -- 100% OVK, sekaligus arah akun SALAH
                           (coa_asal, seharusnya coa_tujuan utk penambah hutang lewat memo -- lihat
                           rationale V2), sudah tercakup benar oleh #v2_saldo_inv (V2-PORTED block,
                           cabang 'Memorial'/'Koreksi Tambahan Hutang OVK'/'DN'). DOC/PAKAN-nya
                           sendiri TIDAK PERNAH ditangani branch lama ini -- kini baru ditambahkan
                           lewat #v2_saldo_inv juga. */
                        ) inv_raw
                        ".$where_jenis_hutang."
                    ) inv
                    left join
                        (
                            ...
                            (
                                ...
                                where
                                    rp.tgl_bayar < '".$start_date."' and
                                    /* DOC/PAKAN/VOADIP dikecualikan -- sudah tercakup lengkap (transfer+
                                       potongan+uang_muka+cn+dn+PPh+pembulatan) di #v2_pembayaran
                                       (V2-PORTED block). Branch generik ini tetap jalan utk PLASMA & jenis
                                       lain yg tidak difilter di sini. */
                                ...
                                /* V2-PORTED: DOC + PAKAN + OVK saldo awal payment, pre-agregasi per
                                   nomor -- lihat #v2_pembayaran di preamble. Ditaruh di kolom transfer
                                   (netral, semua kolom byr di-SUM jadi 1 angka di outer select). */
                                select
                                    nomor,
                                    0 as cn,
                                    0 as dn,
                                ...
                                /* BAYAR LEWAT MEMO (OVK) DIHAPUS -- 100% OVK (coa_tujuan selalu salah
                                   satu dari 2 akun hutang OVK), sudah tercakup di #v2_pembayaran
                                   (V2-PORTED block, cabang 'Pelunasan Memorial (Persediaan OVK)').
                                   DOC/PAKAN-nya sendiri (coa_tujuan=21180.200/21180.100) TIDAK PERNAH
                                   ditangani branch lama ini -- kini baru ditambahkan lewat #v2_pembayaran
                                   juga (Pembayaran Memorial/Pelunasan Memorial (Persediaan Pakan)/
                                   Pembalik Memorial). */
                            ) byr
                            group by
                                byr.nomor
                    ...
                    from (
                        select * from (
                        /* V2-PORTED: DOC + PAKAN + OVK transaksi bulan berjalan -- lihat #v2_sumber_hutang
                           di preamble. kode_trans diisi V2's jenis_trans (label deskriptif -- Konfirmasi
                           Pembayaran DOC/Pakan/OVK, Memorial, DN, Koreksi Tambahan Hutang OVK -- drpd
                           no_bbm mentah spt sebelumnya). */
                        select tanggal, nomor, supplier, total, jenis_trans as kode_trans, jenis_hutang
                        from #v2_sumber_hutang
                        ...
                        /* INVOICE LEWAT MEMO (OVK) DIHAPUS -- 100% OVK, sekaligus arah akun SALAH
                           (coa_asal, bukan coa_tujuan), sudah tercakup benar via #v2_sumber_hutang
                           di atas. DOC/PAKAN-nya TIDAK PERNAH ditangani branch lama ini -- kini baru
                           ditambahkan lewat #v2_sumber_hutang juga. */
                        ) inv_raw
                        ".$where_jenis_hutang."
                    ) inv
                    group by
                    ...
                    (
                        ...
                        select
                            ...
                            (case
                                ...
                                when max(rpd.transaksi) = 'VOADIP' then
                                    (case
                                        when (case when rp.supplier is not null and rp.supplier <> '' then rp.supplier when rp.peternak is not null and rp.peternak <> '' then rp.peternak when rp.ekspedisi is not null and rp.ekspedisi <> '' then rp.ekspedisi end) = '19B004' then 'OVK ORP'
                                        else 'OVK NON ORP'
                                    end)
                                else 'LAINNYA'
                            end) as jenis_hutang
                        ...
                        where
                            ...
                            /* DOC/PAKAN/VOADIP dikecualikan -- sudah tercakup lengkap (transfer+potongan+
                               uang_muka+cn+dn+PPh+pembulatan+koreksi memorial) di #v2_pembayaran
                               (V2-PORTED block) di bawah. Branch generik ini tetap jalan utk PLASMA & jenis
                               lain yg tidak difilter di sini. */
                            rpd.transaksi not in ('DOC', 'PAKAN', 'VOADIP')
                        group by
                            rp.tgl_bayar,
                        ...
                        /* V2-PORTED: DOC + PAKAN + OVK pembayaran bulan berjalan -- lihat #v2_pembayaran
                           di preamble. kode_trans diisi V2's jenis_trans (label deskriptif -- Realisasi
                           Pembayaran, CN, PPh, Pembulatan, Pelunasan/Pembayaran/Pembalik Memorial dst). */
                        select p.tanggal as tanggal, p.supplier, p.nomor as nomor, 0 as debet, p.total as kredit, p.jenis_trans as kode_trans, p.jenis_hutang as jenis_hutang
                        ...
                        /* BAYAR LEWAT MEMO (OVK) DIHAPUS -- 100% OVK (coa_tujuan selalu salah satu dari
                           2 akun hutang OVK), sudah tercakup di #v2_pembayaran (V2-PORTED block, cabang
                           'Pelunasan Memorial (Persediaan OVK)'). DOC/PAKAN-nya sendiri TIDAK PERNAH
                           ditangani branch lama ini -- kini baru ditambahkan lewat #v2_pembayaran juga. */
                        ) byr_raw
                        ".$where_jenis_hutang."
                    ) byr
                    group by
