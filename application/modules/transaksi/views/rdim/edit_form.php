								...
								<tr class="child v-center <?php echo $inactive; ?>" data-key="<?php echo $_key ?>" data-old='<?php echo json_encode($data_old); ?>' >
												...
												<?php 
													...
													if ( isset($data_details[$_key][$index]['tanggal']) ) {
														if ( $mitra['mitra_id'] == $data_details[$_key][$index]['id_mitra'] ) {
															$selected = 'selected';
															$list_kdg = $mitra['kandangs'];
														}
													}
												?>
												<option value="<?php echo $mitra['mitra_id'] ?>" data-jenis="<?php echo $mitra['jenis'] ?>" data-kandangs='<?php echo json_encode($mitra['kandangs']) ?>' <?php echo $selected; ?> ><?php echo $mitra['nama'].' '.$unit; ?></option>
											<?php endforeach; ?>
										</select>
									</td>
									<td class="page0 batas_kanan">
										<select class="form-control" name="kandang" data-kdg="<?php echo $data_details[$_key][$index]['kandang']; ?>">
											<option value="">-</option>
											<?php if ( !empty($list_kdg) ) { ?>
												<?php foreach ($list_kdg as $k_lk => $v_lk) { ?>
													<?php
														$selected = null;
														if ( (int)$v_lk['nomor'] == $data_details[$_key][$index]['kandang'] ) {
															$selected = 'selected';
															$nim = $v_lk['nim'];
														}	
													?>
													<option value="<?php echo $v_lk['id']; ?>" <?php echo $selected; ?> ><?php echo $v_lk['nomor']; ?></option>
												<?php } ?>
											<?php } ?>
										</select>
									</td>
									...
									<td class="page1"><input class="form-control text-right" type="text" name="kapasitas_kandang" value="<?php echo angkaRibuan($kapasitas) ?>" data-tipe="integer" readonly></td>
									...
									<td class="page2">
										<select class="form-control" name="pengawas">
											...
											<?php foreach ($perwakilan['parent']['kanit'] as $k_kanit => $v_kanit): ?>
												...
												?>
												<option value="<?php echo $v_kanit['nik']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($v_kanit['jabatan'].' | '.$v_kanit['nama']); ?></option>
											<?php endforeach ?>
										</select>
									</td>
									<td class="page2">
										<select class="form-control" name="tim_sampling">
											...
											<?php foreach ($perwakilan['parent']['ppl'] as $k_ppl => $v_ppl): ?>
												...
												?>
												<option value="<?php echo $v_ppl['nik']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($v_ppl['jabatan'].' | '.$v_ppl['nama']); ?></option>
											<?php endforeach ?>
										</select>
									</td>
									<td class="page2">
										<select class="form-control" name="tim_panen">
											...
											<?php foreach ($perwakilan['parent']['marketing'] as $k_marketing => $v_marketing): ?>
												...
												?>
												<option value="<?php echo $v_marketing['nik']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($v_marketing['jabatan'].' | '.$v_marketing['nama']); ?></option>
											<?php endforeach ?>
										</select>
									</td>
									<td class="page2">
										<select class="form-control" name="koordinator_area">
											...
											<?php foreach ($perwakilan['parent']['koordinator'] as $k_koordinator => $v_koordinator): ?>
												...
												?>
												<option value="<?php echo $v_koordinator['nik']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($v_koordinator['jabatan'].' | '.$v_koordinator['nama']); ?></option>
											<?php endforeach ?>
										</select>
									</td>
									...
									<td class="page2">
										<select class="form-control" name="formatPb" data-f="<?php echo $data_details[$_key][$index]['format_pb']; ?>" data-nomor="<?php echo trim($data_details[$_key][$index]['nomor_sk']); ?>">
											...
											<?php foreach ($perwakilan['parent']['formatPb'] as $format): ?>
												...
												?>
												<option value="<?php echo $format['id'] ?>" data-perusahaan="<?php echo $format['perusahaan']; ?>" data-pola="<?php echo $format['pola']; ?>" data-nomor="<?php echo trim($format['nomor']); ?>" <?php echo $selected; ?> ><?php echo strtoupper($format['nomor']); ?></option>
											<?php endforeach; ?>
										</select>
										<div class="btn-ctrl">
											<span onclick="rdim.removeRowChild(this)" class="btn_del_row_2x"></span>
