...
class TSDRHPP extends Public_Controller {
    ...
    public function get_data_pakan($noreg)
    {
        ...
        if ( $d_conf->count() > 0 ) {
            ...
            foreach ($d_conf as $k => $value) {
                ...
                $key_plasma = str_replace('-', '', $value['tgl_trans']).' | '.$value['no_sj'].' | '.$value['kode_barang'];
                ...
                $key_inti = str_replace('-', '', $value['tgl_trans']).' | '.$value['no_sj'].' | '.$value['kode_barang'].' | '.$value['hrg_beli'];
                ...
                $hrg_oa = $value['oa'];
                $key = $value['no_sj'].' | '.$value['kode_barang'].' | '.$hrg_oa;
    ...
    public function get_data_pindah_pakan($noreg)
    {
        ...
        $sql = "
            select 
                -- dsts.*, dss.oa, dss.hrg_beli, kp.no_sj, brg.nama as nama_barang, kp.ekspedisi, kp.no_polisi 
                dsts.tgl_trans,
                dsts.kode_trans,
                sum(dsts.jumlah) as jumlah,
                dsts.kode_barang,
                dsts.tbl_name,
                dss.oa, dss.hrg_beli, kp.no_sj, brg.nama as nama_barang, kp.ekspedisi, kp.no_polisi
