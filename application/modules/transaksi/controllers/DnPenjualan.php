...
class DnPenjualan extends Public_Controller {

    private $path = 'transaksi/dn_penjualan/';
    ...
    public function index($segment=0)
    {
        if ( $this->akses['a_view'] == 1 ) {
            $this->add_external_js(array(
                ...
                "assets/transaksi/dn_penjualan/js/dn-penjualan.js",
            ));
            $this->add_external_css(array(
                "assets/select2/css/select2.min.css",
                "assets/transaksi/dn_penjualan/css/dn-penjualan.css",
            ));
            ...
            // Load Indexx
            $data['title_menu'] = 'Debit Note Penjualan';
    ...
    public function getLists()
    {
        ...
        $sql_jenis_dn = "and (";
        $jml_jenis = count($this->jenis_dn);
        $idx = 1;
        foreach ($this->jenis_dn as $k_jd => $v_jd) {
            $sql_jenis_dn .= "d.nomor like '%".$k_jd."%'";
            if ( $idx < $jml_jenis ) {
                $sql_jenis_dn .= " or ";
            }
        ...
        if (  stristr($params['pelanggan'], 'all') === FALSE  ) {
            $sql_query_pelanggan = "and d.pelanggan = '".$params['pelanggan']."'";
        }
    ...
    public function save()
    {
        ...
        try {
            ...
            $m_dn->pelanggan = $params['pelanggan'];
            $m_dn->mitra = (isset($params['mitra']) && !empty($params['mitra'])) ? $params['mitra'] : null;
