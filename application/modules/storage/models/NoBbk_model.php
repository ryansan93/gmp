...
class NoBbk_model extends Conf{
	...
	public function getKodeKeluar($kode, $tanggal){
		...
		$sql = "
			SELECT 
				case
					when exists( select * from no_bbk where SUBSTRING(kode, 0, (LEN('".$kode."')+1+4)) = '".$kode."'+'".$periode."' and SUBSTRING(kode, (LEN('".$kode."')+1+4), 1) >= 3 ) then
	...
	public function getKodeMasuk($kode, $tanggal){
		...
		$sql = "
			SELECT 
				case
					when exists( select * from no_bbk where SUBSTRING(kode, 0, (LEN('".$kode."')+1+4)) = '".$kode."'+'".$periode."' and SUBSTRING(kode, (LEN('".$kode."')+1+4), 1) <= 2 ) then
						'".$kode."'+'".$periode."'+replace(str(substring(coalesce(max(kode),'0000'), (LEN('".$kode."')+1+4), 4)+1, 4), ' ', '0')
					else
						'".$kode."'+'".$periode."'+replace(str(substring(coalesce(max(kode),'0000'), (LEN('".$kode."')+1+4), 4)+1, 4), ' ', '0')
				end as nextId
